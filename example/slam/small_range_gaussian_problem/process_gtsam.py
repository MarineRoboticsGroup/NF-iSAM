...
if __name__ == '__main__':
    ...
    case_list = [setup_folder + '/' + dir for dir in os.listdir(setup_folder) if os.path.isdir(setup_folder + '/' + dir)]
    # case_list = [f"{setup_folder}/case1"]
    gtsam_folder = "gtsam"
    compute_mmd = False
    pose_space = "SE2"
    sample_num = 5000
    ...
    for case_dir in case_list:
        dir = f"{case_dir}/{gtsam_folder}"
        step_list = []
        step_num = 0
        order_file = f"{dir}/step{step_num}_ordering"
        marg_file = f"{dir}/step_{step_num}_marginal"
        joint_file = f"{dir}/step_{step_num}_joint"
        while(os.path.exists(order_file) and
              os.path.exists(marg_file) and
              os.path.exists(joint_file)):
            print("process ",joint_file)
            print(f"step {step_num}")
            # nodes = getVars(order_file=order_file)
            nodes = getVars(order_file=order_file,pose_space="SE2")
            joint = np.genfromtxt(joint_file)
            if len(joint) > 0:
                var2mean = getMeans(nodes, marg_file)
                samples = getSamples(nodes, var2mean, joint, sample_num)
                np.savetxt(fname=f"{dir}/step{step_num}", X=samples)
                step_list.append(step_num)
                if save_plot:
                    plt.figure()
                    ax = plot_2d_samples(samples_array=samples,
                                         variable_ordering=nodes,
                                         show_plot=False, equal_axis=False,legend_on=True, has_orientation=False, title=f'step{step_num}')
                    plt.savefig(f"{dir}/step{step_num}.png")
                    plt.close()
            step_num += 1
            order_file = f"{dir}/step{step_num}_ordering"
            marg_file = f"{dir}/step_{step_num}_marginal"
            joint_file = f"{dir}/step_{step_num}_joint"
        np.savetxt(fname=f"{dir}/step_list", X=np.array(step_list))