...
def batch_factor_graphs(subdir, seed, p_range=0.0, p_da=.0, p_nh=.0,
    ...
    sim_env.set_landmark_area_feasibility([(2, 2), (7, 7)], 0)
    ...
    sim_env.add_landmark(lmk2,7,8)
    sim_env.add_landmark(lmk3,8,1)
    ...
    for seed in range(1, 9):
        ...
        for std in rstds:
            batch_factor_graphs(res_folder, seed, p_range=1, p_da=df_pada, p_nh=0,
                                range_std=std, max_ada_lmk=df_mada, odom_std_scale=df_ostd)
        for std in ostds:
            batch_factor_graphs(res_folder, seed, p_range=1, p_da=df_pada, p_nh=0,
                                range_std=df_rstd, max_ada_lmk=df_mada, odom_std_scale=std)
        # for lmk in ada_lmks:
        #     batch_factor_graphs(res_folder, seed, p_range=1, p_da=df_pada, p_nh=0,
        #                         range_std=df_rstd, max_ada_lmk=lmk, odom_std_scale=df_ostd)