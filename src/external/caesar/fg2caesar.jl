...
function caesar_solve(case_f, inc_step, post_sample)
...
parent_path = "/home/chad/codebase/NF-iSAM/example/slam/problems_for_paper/small_range_gaussian_problem/data_association"
case_folders = readdir(parent_path, join=true)
case_folders = case_folders[isdir.(case_folders)]
# step for grouping variables for incremental inference
inc_step = 1

# create working folders
for case_f in case_folders
    fg_path = string(case_f, "/factor_graph.fg")
    if isfile(fg_path)
        caesar_solve(case_f, inc_step, 1000)
    end
end
