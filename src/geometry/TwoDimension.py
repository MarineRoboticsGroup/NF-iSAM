...
class SE2Pose(object):
    ...
    def grad_xi_expmap(self):
        """
        d(x, y, theat)/d(v1, v2, alpha) where (v1, v2, alpha) is a tangent space vector
        """
        logmap_x, logmap_y, logmap_th = self.log_map()
        res = np.identity(3)
        if(abs(logmap_th) < 1e-5):
            ...
            res[:2, :2] = np.array([[np.sin(logmap_th), np.cos(logmap_th) - 1], [1-np.cos(logmap_th), np.sin(logmap_th)]]) / logmap_th
            dres00dth = (np.cos(logmap_th) * logmap_th - np.sin(logmap_th) ) / logmap_th**2
            dres01dth = (-np.sin(logmap_th) * logmap_th - np.cos(logmap_th) + 1) / logmap_th**2
            res[:2, 2] = np.array([[dres00dth, dres01dth],[-dres01dth, dres00dth]]) @\
                         np.array([logmap_x, logmap_y])
            return res
